...
<?php 
	...
	if ($result->num_rows == 1) {
		...
		if($ktp == ''){
			...
			Upload gambar KTP (max 1mb):<br>
			<input type="file" name="image"/><br>
			<input type="submit" value="Verifikasi KTP" name="submit"/>
			</form>
			<?php
		}else{
			echo ' <img src="data:image/jpeg;base64,'.base64_encode( $ktp ).'" height="80%" width="50%"  /> ';
		}
		echo ' <div class="keteranganproduk"> ';
		echo ' <div class="contentbrg"> Username &nbsp  :'.$username.'</div>' ;
		echo ' <div class="contentbrg"> Nama &nbsp &nbsp &nbsp &nbsp &nbsp:'.$nama.'</div>' ; 
		echo ' <div class="contentbrg"> Stok &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp:'.$saldo.'</div>' ; 
		echo '</div>';
		echo '</div>';
		}
    }
